...
public class ZipUtilitiesTest
{
    ...
    public void zipDirectoryToFileTest()
    {
        ...
        try {
            ...
            assertTrue(zipLength < totalSize, "The ZIP file (" + targetFile
                + ") is larger (" + zipLength
                + ") than the original files: " + totalSize);
    ...
    public void zipFileToFileTest()
    {
        ...
        try {
            ...
            assertTrue(zipLength < totalSize, "The ZIP file (" + targetFile
                + ") is larger (" + zipLength
                + ") than the original file: " + totalSize);
    ...
    /**
     * {@link ZipUtilities#zip(byte[])} followed by {@link
     * ZipUtilities#unzip(byte[])} must produce a byte-for-byte identical result
     * (round-trip identity per the deflate / inflate contract).
     */
    @Test
     ...
     * non-zero-length output (it includes the deflate header/footer at
     * minimum).
     */
    @Test
    ...
    /**
     * GZip output must include the standard GZip magic bytes ({@code 0x1F
     * 0x8B}) at the start of the byte stream.
     */
    @Test
     ...
     */
    @Test
    public void zipUnzipStreamVariantsRoundTrip()
        throws IOException
    {
    ...
    /**
     * {@code ZipUtilities.main} with one or more text arguments (none of which
     * are existing files) must run the deflate/gzip round-trip diagnostics and
     * report success on stdout without throwing or calling {@link System#exit}.
     */
    @Test
     ...
     */
    @Test
    public void mainWithDirAndZipTargetCreatesArchive()
        throws Exception
    {
    ...
    /**
     * {@code ZipUtilities.main} with multiple text arguments must iterate
     * through each, printing both the zip and gzip round-trip diagnostics for
     * every argument.
     */
    @Test
    @Execution(ExecutionMode.SAME_THREAD)
     ...
     * the test sentences enough times that compression provides a meaningful
     * size reduction in the assertions above.
     */
