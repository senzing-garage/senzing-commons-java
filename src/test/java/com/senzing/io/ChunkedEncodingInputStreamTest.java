...
public class ChunkedEncodingInputStreamTest
{
    ...
    private static byte[] encodeChunks(String text)
    {
        try {
            ...
            while (remaining > 0) {
                int count
                    = Math.min((PRNG.nextInt(minChunkSize) + 2) * 2, remaining);
    ...
    public void chunkedEncodingAvailableTest()
    {
        ...
        try {
            ...
            ChunkedEncodingInputStream ceis
                = new ChunkedEncodingInputStream(bais);
    ...
    public void chunkedEncodingCloseTest()
    {
        try {
            ...
            assertFalse(tis.isClosed(),
                        "Pre-condition false, test stream closed.");
            ceis.close();
            assertTrue(tis.isClosed(),
                       "Test stream not closed via chunked stream");
        } catch (Exception e) {
            e.printStackTrace();
