...
public class PropertyReflectorTest
{
    ...
    public List<Arguments> getPropertiesTestParameters()
    {
        ...
        result.add(arguments(Shape.class, Set.of("area", "perimeter"),
                             Set.of()));
      ...
      Class expectedFailure)
    {
        try {
            Object result
                = propertyReflector.getPropertyValue(target, propertyKey);

            // check if an exception was expected
            if (expectedFailure != null) {
                fail("Expected an exception when getting property ("
    ...
    public List<Arguments> getSetPropertyValueTestParameters()
    {
        ...
        result.add(arguments(propRef, newRectangle(), "length", 22.0F,
                             classCast));

        result.add(arguments(propRef, newRectangle(), "length", "22.0",
                             classCast));

        result.add(arguments(propRef, newRectangle(), "area", 200.0,
                             unsupported));

        result.add(
            arguments(propRef, newRectangle(), "perimeter", 60.0, unsupported));

        result.add(arguments(propRef, newRectangle(), "volume", null,
                             illegalArg));
    ...
    {
        try {
            propertyReflector.setPropertyValue(target, propertyKey,
                                               propertyValue);

            String propVal = (propertyValue == null)
                ? String.valueOf(null) : (propertyValue.getClass().getName()
                                          + ": " + propertyValue);

            // check if an exception was expected
            if (expectedFailure != null) {
                fail("Expected an exception when setting property ("
    ...
    /**
     * Bean with a property that has only a setter (write-only property). Used
     * to exercise the {@code getPropertyValue} branch where the key exists in
    ...
    /**
     * Bean exercising every {@code switch} case in {@code
     * PropertyReflector.buildJsonObject} for the JSON-type dispatch — Integer /
     * Short / Long / Float / Double / Boolean / BigDecimal / BigInteger /
    ...
    /**
     * For a read-only property (no setter), {@link
     * PropertyReflector#setPropertyValue} must throw {@link
     * UnsupportedOperationException} per the existing Rectangle
     * "area"/"perimeter" cases. Adds a regression-style assertion using the
     * dedicated {@code Address} fixture which is already entirely read-only.
     */
    @Test
     ...
     * isAssignableFrom fallback must select an overload whose parameter type is
     * a supertype of the value's type — here, passing a {@link Date} to a class
     * that has only {@code setTag(String)}, {@code setTag(int)}, and {@code
     * setTag(Object)} must select the Object overload.
     */
    @Test
    ...
    public void setPropertyValueRejectsNullForPrimitiveOnlySetter()
    {
        PropertyReflector<PrimitiveOnlySetterBean> reflector
            = PropertyReflector.getInstance(PrimitiveOnlySetterBean.class);
    ...
    /**
     * {@link PropertyReflector#toJsonObject(Object)} on a Java-bean with one
     * accessor per documented {@code switch} case must produce a JSON object
     * whose keys are the property names and whose values match the
     * corresponding JSON-type representation for each Java type (Integer,
     * Short, Long, Float, Double, Boolean, BigDecimal, BigInteger, String,
     * Collection, array).
     */
    @Test
    ...
    /**
     * A {@link Map} input with non-String keys must NOT be treated as a
     * key/value JSON object — instead, the implementation falls through to bean
     * reflection on the {@link Map} class itself (which produces an
     * empty/near-empty result, since {@link Map}'s own getters are inherited
     * from {@link Object} and skipped).
     */
    @Test
