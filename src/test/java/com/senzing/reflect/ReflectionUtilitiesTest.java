...
public class ReflectionUtilitiesTest
{
    ...
    /**
     * Test interface used by the {@code restrictedProxy} tests below. Two
     * methods so we can restrict one and verify the other still delegates to
     ...
     * Method...)} must throw {@link NullPointerException} when given a null
     * class loader.
     */
    @Test
     ...
     */
    @Test
    public void restrictedProxyAddsAdditionalRestriction()
        throws Exception
    {
        RestrictableImpl target = new RestrictableImpl();
        java.lang.reflect.Method restrictedMethod
     ...
     * construct method pairs that share a name but differ in return type,
     * arity, or parameter type) and a separate {@code beta} (different name).
     * {@link CompB} provides an {@code alpha} with a different return type from
     * {@link CompA}'s — used to exercise the "same name, different return
     * types" branch since Java disallows return-type-only differences within a
     * single declaring class.
     */
    ...
    /**
     * Reflectively grab the private {@code METHOD_COMPARATOR} for direct
     * testing. Wrapped in an unchecked rethrow so the test methods don't have
    ...
    /**
     * When two methods share name and return type but differ in parameter
     * count, the comparator must order them by parameter count (longer
     * parameter list sorts later).
     */
    @Test
    ...
    /**
     * When two methods share name, return type, and arity but differ in
     * parameter type, the comparator must order them by the parameter type's
     * name.
     */
    @Test
