...
public interface CommandLineOption<
        T extends Enum<T> & CommandLineOption<T, B>,
        B extends Enum<B> & CommandLineOption<B, ?>>
{
   ...
   * This is useful when a flag changes and the old version of it is deprecated
   * but still supported. It is <b>NOT</b> allowed to pass multiple flags for
   * the same option.
   *
   ...
   * environment for this option is allowed, however, they are taken in order of
   * preference (first the primary environment variable and then each of those
   ...
   * environment variables for this option. These are environment variables that
   * are only checked if another option is specified that is dependent on this
   * option and this option is missing. The environment variables in the
   ...
   * sets of options that this option depends on. At least one of the contained
   ...
   * are allowed depends on the {@linkplain #getConflicts() conflicts} for each
   * option.
   *
  ...
  /**
   * Checks if the command-line option has a value that is considered sensitive
   ...
   * command line option. If this returns a negative number then parameters are
   * read until the next recognized command line option is read after consuming
   * at least the {@linkplain #getMinimumParameterCount()} minimum number of
   ...
   * the parameters that are set if none are specified. If no default parameters
   * then this returns <code>null</code>. An empty {@link List} indicates that
   * the option is specified by default but with no parameters. The default
