...
/**
 * Interface for {@link Enum} classes that represent command line options to
 * enable them to be used with {@link CommandLineUtilities}.
 *
 ...
 *            option type has no base and returns <code>null</code> from {@link
 *            #getBaseOptionType()}.
 */
    ...
    B extends Enum<B> & CommandLineOption<B, ?>>
{
     ...
     * returned is the base generic parameter type or the same type of this
     * class if no base.
     *
     ...
     *
     * @return The <b>unmodifiable</b> {@link Set} of {@link String} synonym
     *         flags for this optional, or an empty set if there are no
     *         synonyms.
     */
    ...
    /**
     * Gets the environment variable that can be used to set the value for this
     * option, or <code>null</code> if the value for this option cannot be set
     ...
     * that are only checked if another option is specified that is dependent on
     * this option and this option is missing. The environment variables in the
     * returned {@link List} are checked in order until the first one is found.
     * These are essentially environment synonyms that are only conditionally
    ...
    /**
     * Gets the {@link Set} of {@link Set} instances describing combinations of
     * sets of options that this option depends on. At least one of the
     * contained {@link Set} instances must be satisfied for this option to be
     * used.
     *
     * @return The {@link Set} of {@link Set} instances describing combinations
     *         of sets of options that this option depends on.
     */
    ...
    /**
     * Returns the minimum number of additional parameters that should follow
     * this command line option. This should return a non-negative number and at
     * least this many parameters will be consumed. By default this returns zero
     * (0).
     *
     * @return The minimum number of additional parameters that should follow
    ...
    /**
     * Returns the maximum number of additional parameters that should follow
     * this command line option. If this returns a negative number then
     * parameters are read until the next recognized command line option is read
     * after consuming at least the {@linkplain #getMinimumParameterCount()}
     * minimum number of parameters}. By default this returns negative one (-1).
     *
     * @return The maximum number of additional parameters that should follow
     *         this command line option, or a negative number to indicate that
    ...
    /**
     * Returns the default parameter values associated with the option. These
     * are the parameters that are set if none are specified. If no default
