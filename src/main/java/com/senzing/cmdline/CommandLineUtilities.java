...
public class CommandLineUtilities {
     ...
     *                  extend,
     *                  <b>OR</b> the same as type <code>T</code> if the
     *                  command-line
     ...
     */
    public static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void putValue(
        ...
        if (!checkNoArgFalse(value) && (value.getSource() != DEFAULT)) {
            ...
            for (CommandLineOption<?, ?> opt : optionKeys) {
                ...
                if ((conflicts != null && conflicts.contains(opt))
                    ...
                    if (conflictValue.getSource() == DEFAULT)
                        continue;

                    // no conflict if the other is a no-arg option with a false value
                    if (checkNoArgFalse(conflictValue))
                        continue;
     ...
     *                     <b>OR</b> the same as type <code>T</code> if the
     *                     command-line
     *                     option type has no base and returns <code>null</code>
     *                     from
     *                     {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> boolean checkNoArgFalse(
            CommandLineValue cmdLineValue) {
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    public static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> T lookup(
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> Set<CommandLineOption> getPrimaryOptions(
            Class<T> enumClass) {
        // create the result
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void populatePrimaryOptions(
            Set<CommandLineOption> set, Class<T> enumClass) {
     ...
     *                  {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> Set<Class<? extends CommandLineOption>> getTypeChain(
            Class<T> enumClass) {
     ...
     *                  {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void populateTypeChain(
     ...
     *                  {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void populateOptionsChain(
            Set<CommandLineOption> set,
            Class<T> enumClass) {
        if (enumClass == null)
            return;
        EnumSet<T> enumSet = EnumSet.allOf(enumClass);
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    public static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> List<DeprecatedOptionWarning> validateOptions(
            Class<T> enumClass,
        ...
        Set<Class<? extends CommandLineOption>> typeChainSet = getTypeChain(enumClass);
        ...
        for (CommandLineValue cmdLineValue : optionValues.values()) {
            ...
            if (cmdLineValue.getSource() == DEFAULT)
                continue;
     ...
     * @param processor           The {@link ParameterProcessor} to use for handling
     *                            the
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    public static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> Map<CommandLineOption, CommandLineValue> parseCommandLine(
            Class<T> enumClass,
            String[] args,
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> Map<CommandLineOption, CommandLineValue> parseCommandLine(
            Class<T> enumClass,
            String[] args,
            ParameterProcessor processor,
            boolean ignoreEnvironment,
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (minParamCount == 0 && maxParamCount == 0
                ...
                if (bound > args.length)
                    bound = args.length;
                for (int nextIndex = index + 1; nextIndex < bound
                        && !lookupMap.containsKey(args[nextIndex]); nextIndex++) {
                    params.add(args[nextIndex]);
                    index++;
                }

                // check if there are more (unexpected) parameters
                for (int nextIndex = index + 1; (nextIndex < args.length && !args[nextIndex].startsWith("-")
                        && !lookupMap.containsKey(args[index])); nextIndex++) {
                    params.add(args[nextIndex]);
                    index++;
                }
        ...
        try {
            List<DeprecatedOptionWarning> warnings = validateOptions(enumClass, result);
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void processEnvironment(
            Class<T> enumClass,
            ParameterProcessor processor,
            Map<CommandLineOption, CommandLineValue> optionValues)
            throws CommandLineException {
     ...
     *            {@link CommandLineOption#getBaseOptionType()}.
     */
    private static <T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>> void processDefaults(
            Class<T> enumClass,
            ParameterProcessor processor,
            Map<CommandLineOption, CommandLineValue> optionValues)
            throws CommandLineException {
     ...
     *                        values if specified, if <code>null</code> a new {@link
     *                        Map} is created and returned.
     * @param stringBuilder   If not <code>null</code>then this
     *                        {@link StringBuilder}
    ...
    public static Map<CommandLineOption, Object> processCommandLine(
        ...
        Map<CommandLineOption, Object> result = (processedValues == null) ? new LinkedHashMap<>() : processedValues;
