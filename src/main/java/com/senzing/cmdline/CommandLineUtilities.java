...
public class CommandLineUtilities
{
    ...
    /**
     * Returns a new {@link String} array that contains the same elements as the
     ...
     * checking to ensure that the option is NOT already specified and checking
     * if the option has any conflicts. If the specified option is deprecated
        ...
        if (!checkNoArgFalse(value) && (value.getSource() != DEFAULT)) {
            ...
            for (CommandLineOption<?, ?> opt : optionKeys) {
                ...
                if ((conflicts != null && conflicts.contains(opt))
                    ...
                    // no conflict if the other is a no-arg option with
                    // a false value
    ...
    /**
     * Validates the specified {@link Set} of specified {@link
     * CommandLineOption} instances and ensures that they logically make sense
     * together. This checks for the existing of at least one primary option (if
            ...
            throws IllegalArgumentException, CommandLineException
    {
        ...
        for (CommandLineValue cmdLineValue : optionValues.values()) {
            ...
            CommandLineOption option
                = (CommandLineOption) cmdLineValue.getOption();
     ...
     */
    private static Map<String, CommandLineOption> createFlagLookupMap(
            Class enumClass)
    {
        // get all the options
     ...
     * command-line options that are not explicitly specified on the command
     * line by using environment variables for those options have environment
     * variable equivalents defined. Use of the environment is disabled if the
     * <code>ignoreEnvOption</code> parameter is non-null and that option is
            ...
            throws CommandLineException
    {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (minParamCount > 0) {
                ...
                // back up the index by one so when we advance it
                // later it is correct
                index--;
            }
            ...
            throws CommandLineException
    {
        ...
        for (CommandLineOption primaryOption : primaryOptions) {
            ...
            if (!optionValues.containsKey(primaryOption)) {
                List<String> fallbacks
                    = primaryOption.getEnvironmentFallbacks();
        ...
        for (CommandLineOption option : optionKeys) {
            ...
            // iterate over the dependency sets and see if at least
            // one is satisfied
            ...
            if (!satisfied) {
                // find the first dependency set with missing items
                // with fallbacks
                for (Set<CommandLineOption> dependencySet : dependencySets) {
                    ...
                    for (CommandLineOption dependency : dependencySet) {
                        ...
                        List<String> fallbacks
                            = dependency.getEnvironmentFallbacks();
                        if (fallbacks != null && fallbacks.size() > 0) {
                            fallbackCount++;
                        }
                    }
            ...
            throws CommandLineException
    {
        ...
        for (CommandLineOption option : enumSet) {
            // prefer explicit command-line arguments, so skip if
            // already present
            ...
            StringBuilder stringBuilder)
    {
        ...
        optionValues.forEach((key, cmdLineVal) -> {
            ...
            JsonObjectBuilder job2
                = (doJson) ? Json.createObjectBuilder() : null;
            ...
            CommandLineOption... options)
    {
        ...
        // check if we can balance things out if we have a single
        // dangling option
     ...
     * found. This method returns <code>null</code> if the JAR base URL cannot
     * be determined.
     *
