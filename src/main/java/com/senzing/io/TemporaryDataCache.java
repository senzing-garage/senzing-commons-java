...
public class TemporaryDataCache
{
    ...
    public boolean isAppending()
    {
        synchronized (this.fileParts) {
            return this.consumerThread.isAlive()
                && this.consumerThread.isAppending();
        }
    }
    ...
    private class FilePartSink extends Thread
    {
        ...
        private void beginNextFilePart()
        {
            ...
            synchronized (owner.fileParts) {
                try {
                    ...
                    String fileName
                        = baseFileName + "-" + this.partIndex + ".dat";

                    Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM);
                    cipher.init(Cipher.ENCRYPT_MODE, owner.keySpec,
                                owner.ivSpec);
            ...
            throws IOException
        {
            ...
            synchronized (owner.fileParts) {
                ...
                if (this.closed) {
                    throw new IllegalStateException(
                        "Sink thread is already shutdown");
                }
                ...
                synchronized (owner.fileParts) {
                    ...
                    if (!this.isClosed()) {
                            ...
                            && this.currentWriteCount > 0)
                        {
                            ...
                            // milliseconds
                            long duration
                                = (now - this.lastWriteTime) / 1000000L;

                            // check if the duration exceeds the timeout
                            if (duration >= FILE_PART_TIMEOUT) {
                                // if it the timeout is exceeded then complete
                                // the file part
                                this.completeCurrentFilePart();
                                waitTime = FILE_PART_TIMEOUT;
                            } else {
                                // otherwise wait for at least the remaining
                                // amount of time
    ...
    private static class CacheFilePart implements Comparable<CacheFilePart>
    {
        ...
        public boolean equals(Object o)
        {
            ...
            return offset == filePart.offset && length == filePart.length;
        }
    ...
    private class ChainFileInputStream extends InputStream
    {
            ...
            throws IOException
        {
            ...
            while (remainingSkip > 0L) {
                ...
                // check if the remaining number of bytes in the file is less
                // than
                // the remaining number to skip
                long remaining
                    = this.currentFilePart.length - this.currentOffset;
            ...
            throws IOException
        {
            try {
                ...
                synchronized (owner.fileParts) {
                    this.currentFilePart = owner.fileParts.get(
                        this.currentFileIndex);
                    ...
                    if (isDebugLogging()) {
                        ...
                        tmpCipher.init(Cipher.DECRYPT_MODE, owner.keySpec,
                                       owner.ivSpec);
                        try (FileInputStream fis
                                 = new FileInputStream(filePart);
                             CipherInputStream cis
                                 = new CipherInputStream(fis, tmpCipher);
                             GZIPInputStream gis = new GZIPInputStream(cis);
                             InputStreamReader isr
                                 = new InputStreamReader(gis, UTF_8))
                        {
