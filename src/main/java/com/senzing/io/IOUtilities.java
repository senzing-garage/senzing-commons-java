...
public class IOUtilities
{
        ...
        for (CharsetMatch match : matches) {
            ...
            if (bestMatch == null) {
                ...
                if (match.getName().toUpperCase().startsWith("UTF")) {
                    ...
                    // if not UTF, then check if the next one has equal
                    // confidence
                    continue;
                }
            }
     ...
     * order mark" (BOM) character at the beginning of the file for UTF
     * character encodings (e.g.: "UTF-8", "UTF-16" or "UTF-32"). If the
     ...
     * @return <code>true</code> if the files differ, otherwise
     *         <code>false</code>
     *
     * @throws IOException If an I/O exception occurs.
     */
     ...
     * @param file1 The first file.
     * @param file2 the second file.
     * @param timestampSignificant <code>true</code> if timestamps need to be
     ...
     * close the underlying {@link OutputStream} when {@link
     * OutputStream#close()} is called -- it will instead call {@link
     * OutputStream#flush()} This means you must keep a reference to the
     * original output stream so that you can close it when appropriate.
     *
     ...
     * underlying {@link Reader} when {@link Reader#close()} is called. This
     * means you must keep a reference to the original reader so that you can
     ...
     *         Reader}, but will not close the backing {@link Reader} when
     *         closed.
     */
     ...
     * underlying {@link Writer} when {@link Writer#close()} is called -- it
     * will instead call {@link Writer#flush()} This means you must keep a
     * reference to the original output stream so that you can close it when
     * appropriate.
     *
     * @param writer The backing output stream to wrap.
     *
     * @return The {@link Writer} that is backed by the specified {@link
     *         Writer}, but will not close the backing {@link Writer} when
     *         closed.
     */
    ...
    /**
     * Extends {@link FilterInputStream} to prevent closing of the backing
     * stream.
     */
