...
public class SQLiteConnector implements Connector
{
    ...
    static {
        ...
        // not use a Connection object (or PreparedStatement) in more
        // than one thread at any given time.  To do this at run-time we
        // need to set SQLiteOpenMode.NOMUTEX.
    ...
    /**
     * The <b>unmodifiable</b> {@link List} of {@link String}
     * <code>PRAGMA</code> statements that are run on the {@link Connection}
     * after opening it. These are as follows:
     * <ol>
    ...
    /**
     * Utility method to check for <code>null</code> file path before
     * constructing the {@link File} object to use.
     *
     * @param filePath The non-null file path.
     * @param connProperties The {@link Map} of {@link String} key to {@link
     *                       String} values for the connection properties, or
    ...
    /**
     * Default constructor using a temporary file that will be deleted upon exit
     * of the executable.
     */
    ...
    /**
     * Gets the {@link File} instance describing the SQLite database file used
     * by this instance.
     *
     ...
     * Connection} to the file with which this instance was constructed. After
     ...
     * the pool for use by a single thread at any given time and then returned
     * to the pool (via {@link Connection#close()}) when a unit of work is
     * complete. The returned {@link Connection} should <b>NOT</b> be used
     * <b>concurrently</b> in multiple threads. This is what SQLite
     ...
     * are thread-safe at the driver level with multiple active {@link
     * Connection} instances to the same database being leveraged in different
     * threads.
     *
        ...
        throws SQLException
    {
        ...
        try {
            conn = DriverManager.getConnection(
                jdbcUrl, SQLITE_CONFIG_PROPERTIES);
    ...
    /**
     * Gets the {@link List} of the SQL statements to run to initialize a SQLite
