...
public class PoolConnectionProvider implements ConnectionProvider
{
   ...
   * The constructed instance will wait indefinitely for a {@link Connection} to
   * become available.
   *
   ...
   * time for waiting for a {@link Connection} from the pool. To specify that no
   * wait should be done, specify zero (0) as the wait time. To specify that the
   * provider should wait indefinitely then specify a negative number.
   *
   * @param pool The {@link ConnectionPool} to use.
