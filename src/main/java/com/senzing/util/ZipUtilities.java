...
public class ZipUtilities
{
    ...
    {
        byte[] compressedData
            = Base64.getDecoder().decode(compressedBase64Data);
        return unzip(compressedData);
    }

    /**
     * Compresses the specified text by converting it to UTF-8 bytes,
     * compressing it with the deflater zlib compression algorithm and encoding
     * the result in Base-64 encoding.
     *
    ...
    /**
     * Decompresses the specified byte array using the GZip compression
     * algorithm.
     *
     * @param compressedData The byte array containing the compressed data.
     * @return The uncompressed byte array.
    ...
    /**
     * Compresses the specified text by converting it to UTF-8 bytes,
     * compressing it with the GZip compression algorithm and encoding the
     * result in Base-64 encoding.
     *
     * @param uncompressedText The byte array containing the uncompressed data.
     * @return The compressed byte array.
     */
     ...
     * @throws IOException If a failure occurs.
     */
    public static void zip(File directoryOrFile,
        ...
        try {
            ...
            if (source == null || target == null) {
                ...
                if (target.exists() && target.isDirectory() && sourceZip) {
                    ...
                    System.err.println(
                        "At least one argument must be a ZIP file");
                    System.exit(1);
                }
            }
        } catch (Exception e) {
