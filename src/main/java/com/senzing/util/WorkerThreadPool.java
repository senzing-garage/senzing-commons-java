...
public class WorkerThreadPool
{
     ...
     * to create.
     *
     * @param baseName The base name to use as a prefix when naming the worker
     *                 threads in the pool.
     *
     ...
     *
     * @throws E If the specified {@link Task#execute()} method throws an
     *           exception.
     *
     * @param <T> The type returned by the worker thread.
     * @param <E> The type of the exception thrown by the worker thread if a
     *            failure occurs.
     */
    public <T, E extends Exception> T execute(Task<T, E> task)
        throws E
    {
        ...
        try {
            ...
            if (thread != null) {
                ...
                synchronized (this.available) {
                    // if not complete then return it to the pool and set thread
                    // to null
                    if (!this.closed) {
                        this.available.add(thread);
                        this.available.notifyAll();

                        // set the thread to null to indicate that it was
                        // returned
                        // to the thread pool
                        thread = null;
                    }
     ...
     * obtaining a thread until the {@link #resume(AccessToken)} method is
     * called (which should occur in a "finally" block typically). If there are
     * outstanding workers executing tasks on threads then this method waits
     * until they are complete.
     *
     * @return The {@link AccessToken} to use for {@linkplain
     *         #resume(AccessToken) unpausing}, or <code>null</code> if already
     ...
     * parameter is <code>null</code> then this method does nothing and if it is
     * the wrong {@link AccessToken} then an {@link IllegalArgumentException} is
     * thrown.
     *
     * @param accessToken The {@link AccessToken} to use to verify unpausing the
     *                    {@link WorkerThreadPool}, or <code>null</code> if the
    ...
    private class WorkerThread extends Thread
    {
        ...
        /**
         * Marks this thread complete so it stops waiting for new tasks to
         * execute.
         */
         ...
         *
         * @return <code>true</code> if this thread is currently busy executing
         *         a task, otherwise <code>false</code>.
         */
