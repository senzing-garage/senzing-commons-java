...
public class AsyncWorkerPool<T>
{
     ...
     */
    private final List<AsyncWorker<T>> available;

    /**
     * The list of all {@link AsyncWorker} instances whether available or not.
     */
    private List<AsyncWorker<T>> allThreads;
    ...
    public AsyncWorkerPool(String baseName, int size)
    {
        ...
        for (int index = 0; index < size; index++) {
            AsyncWorker<T> aw = new AsyncWorker<>();
    ...
    private class AsyncWorker<T> extends Thread
    {
         ...
         * marks this thread as completed, notifies, and joins against the
         * thread. Finally, this method returns the {@link AsyncResult} from the
         * previous task that was executed (if not yet consumed).
         */
            ...
            while (this.isBusy() || (!this.isComplete() && !pool.isClosed())) {
                ...
                if (task != null) {
                    try {
                        ...
                        result = new AsyncResult<>(this.currentTask.execute(),
                                                   null);
                    } catch (Exception e) {
                        // record any failure for the task
                        result = new AsyncResult<>(null, e);
                    } finally {
                        // update worker fields to clear the task and store the
                        // result
                        synchronized (this) {
                            this.currentTask = null;
